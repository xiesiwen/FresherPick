...
Page({
  ...
  seeOrders: function(userInfos) {
    ...
    if (userInfos.uLevel == 1) {
      ...
      const query3 = query.equalTo('distributorId', "==", "")
    ...
    query.find().then(res => {
      ...
      console.log("刷新that.data.orderList")
    })
  ...
  orderCancle: function(e) {
    ...
    that.seeOrders(that.data.userInfos);
    //刷新订单；是因为 异步操作的关系，没有更新that.data
    wx.startPullDownRefresh()
  },
  ...
  startDistributed: function(e) {
    var that = this;
    var orderObjectId = e.detail.value.orderObjectId;
    var distributorId = e.detail.value.distributorIdInput;

    if (!distributorId || !that.checkSumbitMobile(distributorId).flag){
      return ;
    }
  ...
  checkSumbitMobile: function(mobile) {
    var result = {
      "flag": true,
      "msg": ""
    };
  ...
  finishOrder: function(e){
    var that = this;
    var orderObjectId = e.currentTarget.dataset.orderobjectid;

    const query = Bmob.Query('Orders');
    query.get(orderObjectId).then(res => {
      console.log("修改订单 " + orderObjectId + " 为 true")
      res.set('done', true)
      res.save()
    }).catch(err => {
      console.log("修改订单失败，订单 " + orderObjectId)
    })
